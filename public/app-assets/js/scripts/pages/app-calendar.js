...
document.addEventListener("DOMContentLoaded", function () {
    var calendarEl = document.getElementById("calendar"),
        ...
        calendarsColor = {
            partial: "primary",
            expiredUnpaid: "danger",
            sendPartial: "secondary",
            senddelay: "success",
            madarec: "warning",
            delay: "info",
            sadadrec: "secondary",
    ...
    window.urlRole = window.location.pathname.split('/')[1];

    function fetchEvents(info, successCallback) {
        console.log('function fetchEvents(info, successCallback) ');
        // Fetch Events from API endpoint reference
        $.ajax({
            // url: '../../../app-assets/data/app-calendar-events.js',
            url: `/api/admin/partial-pay-event?id=${window.officer_calendar_id ?? ''}&role=${window.urlRole ?? ''}`,
            type: "GET",
            success: function (result) {
                // Get requested calendars as Array
                //   console.log('result of events ====== ', result);
                var calendars = selectedCalendars();
                //   console.log('selectedCalendars()', calendars);

                //   return [result.events.filter(event => calendars.includes(event.extendedProps.calendar))];
                var finalResult = result.filter((event) =>
    ...
    window.setOfficerCalendarId = (event)=>{
        console.log('this is officer id',event.value);
        if(event.value == 'all'){
            window.officer_calendar_id = '';
        }
        else{
            window.officer_calendar_id = event.value;
        }
