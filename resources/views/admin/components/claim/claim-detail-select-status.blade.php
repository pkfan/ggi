...
<div class="card-body">
    ...
    <form method="post" action="{{ url('admin-claim-status') }}">
        ...
        <select class="form-control" name="cstatus" id="sectionChooser">
            {{-- @if ($status != null) --}}
            <option value="1" @if ($status?->status == \App\Enums\ClaimStatus::FOLLOW_UP->value) selected @else selected @endif>
                Follow Up للمتابعة
            </option>

            <option value="2" @if ($status?->status == \App\Enums\ClaimStatus::COLLECTED->value) selected @endif>
                Collected تم التحصيل</option>
            <option value="3" @if ($status?->status == \App\Enums\ClaimStatus::DELAY_SETTLEMENT->value) selected @endif>Delay
                Settlement تأجيل السداد</option>
            <option value="4" @if ($status?->status == \App\Enums\ClaimStatus::PARTIAL_SETTLEMENT->value) selected @endif>Partial
                Settlement سداد جزئ</option>
            <option value="5" @if ($status?->status == \App\Enums\ClaimStatus::TRANSFER_TO_MORROR->value) selected @endif>Transfer to
                Morror تحول
                للمرور
            </option>
            <option value="6" @if ($status?->status == \App\Enums\ClaimStatus::TRANSFER_TO_LAWYER->value) selected @endif>Transfer to
                Lawyer تحول للمحام</option>

            <option value="20" @if ($status?->status == \App\Enums\ClaimStatus::SEND_TO_LEGAL_DEPARTMENT->value) selected @endif>Send to legal department
            </option>
            <option value="21" @if ($status?->status == \App\Enums\ClaimStatus::SEND_TO_COLLECTION_OFFICE->value) selected @endif> Send to collection office
            </option>
            @if(request()->is('supervisor/*'))

            @else
            <option value="22" @if ($status?->status == \App\Enums\ClaimStatus::SEND_BACK_TO_SUPERVISOR->value) selected @endif>Send back to supervisor
            </option>
    ...
    <div class="container">
        @php
        $claimStatus = \App\Models\Claim::where('id',$claim->id)->first();
        @endphp
        @if($claimStatus->status == \App\Enums\ClaimStatus::COLLECTED->value)
        <div class="alert" style="background-color:#5499C7; color:white"> Collected</div>
        @elseif($claimStatus->status == \App\Enums\ClaimStatus::DELAY_SETTLEMENT->value)
        <div class="alert" style="background-color:#5DADE2; color:white;padding:10px"> Dellay Settlement</div>
        @elseif($claimStatus->status == \App\Enums\ClaimStatus::PARTIAL_SETTLEMENT->value)
        <div class="alert" style="background-color:#76D7C4; color:white;padding:10px"> Partial Settlement</div>

        @elseif($claimStatus->status == \App\Enums\ClaimStatus::TRANSFER_TO_MORROR->value)
        <div class="alert" style="background-color:#7DCEA0; color:white;padding:10px"> Trasnfer To Morror</div>
        @elseif($claimStatus->status == \App\Enums\ClaimStatus::TRANSFER_TO_LAWYER->value)
        <div class="alert" style="background-color:#F7DC6F; color:black;padding:10px"> Trasnfer To Lawyer</div>
        @elseif($claimStatus->status == \App\Enums\ClaimStatus::TRANSFER_TO_FINANCE->value)
        <div class="alert" style="background-color:#F8C471; color:black;padding:10px"> Trasnfer To Finance</div>
        @elseif($claimStatus->status == \App\Enums\ClaimStatus::TRANSFER_TO_ELM->value)
        <div class="alert" style="background-color:#E59866; color:white;padding:10px"> Trasnfer To Finance</div>
        @elseif($claimStatus->status == \App\Enums\ClaimStatus::TRANSFER_TO_IC->value)
        <div class="alert" style="background-color:#D5DBDB; color:white;padding:10px"> Trasnfer To IC</div>
        @elseif($claimStatus->status == \App\Enums\ClaimStatus::CLOSE->value)
        <div class="alert" style="background-color:#C39BD3; color:white;padding:10px"> Close</div>
        @elseif($claimStatus->status == \App\Enums\ClaimStatus::COLLECTED_BY_INSURANCE->value)
        <div class="alert" style="background-color:#FEF127; color:white;padding:10px"> Collected By Insurance</div>
        @elseif($claimStatus->status == \App\Enums\ClaimStatus::SEND_TO_LEGAL_DEPARTMENT->value)
        <div class="alert" style="background-color:#9BEAFF; color:balck;padding:10px"> Legal Department </div>
        @elseif($claimStatus->status == \App\Enums\ClaimStatus::SEND_TO_COLLECTION_OFFICE->value)
        <div class="alert" style="background-color:#5B8CFF; color:white;padding:10px"> Collection Office </div>
        @else
        <!-- <div class="alert" style="background-color:#5BFFBE; color:balck;padding:10px"> Collection Office </div> -->
        @endif
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Request To Change Status</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form action="{{route('requestToChangeStatus')}}" method="post">
                            @csrf
                          
                            <input type="hidden" name="claim_id" value="{{$claim->id}}">
                            <label for="current_status">Current Status <b class="text-danger">*</b></label>
                            <input type="text" name="current_status" class="form-control">
                            <label for="new_status">New Status Status <b class="text-danger">*</b></label>
                            <input type="text" name="new_status" class="form-control">
                            <label for="reason">Reason <b class="text-danger">*</b></label>
    ...
    <div id="details-container" @if($claim->status == \App\Enums\ClaimStatus::FOLLOW_UP->value) style=" border: 1px solid #dddddd; padding: 16px; border-radius: 8px; background: #f9f9f9; "@endif>
        ...
        <div class="col-md-12 panel delayedDiv">
            <form method="post" action="{{ route('admin-pay.delay') }}">
                ...
                <select class="form-control" name="type">
                    <option value="mada">Online Pay</option>
                </select><br>
