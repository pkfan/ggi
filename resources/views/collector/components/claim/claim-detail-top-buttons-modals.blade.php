...
<div>
    ...
    <div class="d-inline-block">
        ...
        <a href="{{ route('admin.comment.id', ['id' => $claim->id]) }}">
            <button type="button" class="btn btn-outline-dark btn-sm"
                style="max-width: 115px;background: linear-gradient(-180deg, white, #adadad);"
                data-target="#primary">
    ...
    @permission('call-history')
        <div class="d-inline-block">
            <!-- Button trigger modal -->
            <button type="button" class="btn btn-outline-dark btn-sm"
                style="max-width: 90px;background: linear-gradient(-180deg, white, #adadad);"
                                        ...
                                        @else
                                            ...
                                            @if (isset($response['statusCode']) == false)
                                                ...
                                                @if ($response['status'] != 'completed')
                                                    <a href="{{ url('admin/call-again/' . $claim->id) }}"
    ...
    @permission('sms-history')
        <div class="d-inline-block">
            ...
            <!-- Modal -->
            <div class="modal fade text-left modal-primary" id="success"
                            ...
                            <div class="mb-3">
                                ...
                                @if ($res)
                                    @foreach ($res as $sms)
                                        ...
                                        <a href="{{ url('admin/resend/msg/' . $sms->claim_id.'/'.$sms->id) }}"
                                            class="btn btn-primary">Resend SMS</a>
    ...
    @if ($claim->link != null || $claim->status == 1)
        @permission('create-mada-settlement')
            <div class="d-inline-block">
                ...
                <!-- Modal -->
                <div class="modal fade modal-secondary text-left" id="danger"
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        ...
                        {{-- form start  --}}
                        <form action="{{ route('admin.create-payment-link') }}"
                            method="post">
                            @csrf
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        @if ($payc != 0)
                                            @foreach ($paylinks as $link)
                                                ...
                                                Link:<span
                                                    style="color:blue">{{ $link->link }}</span>
                                                <br>
                                                <?php $clink++; ?>
                                            @endforeach
                                        @endif
        ...
        @permission('sadad-invoice')
            <div class="d-inline-block">
                <!-- Button trigger modal -->
                <button type="button" class="btn btn-outline-dark btn-sm"
                    style="max-width: 100px;background: linear-gradient(-180deg, white, #adadad);"
                    data-toggle="modal" data-target="#warning">
                    @lang('language.Sadad Invoice')
                </button>
                <!-- Modal -->
                <div class="modal fade text-left modal-secondary" id="warning"
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary"
                                    data-bs-dismiss="modal">@lang('language.No')</button>
                                <a href="{{ url('admin/sadad-link/' . $claim->id) }}"
                                    class="btn btn-primary">@lang('language.Yes')</a>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    @permission('finance-report')
        <div class="d-inline-block">
            ...
            <button type="button" class="btn btn-outline-dark btn-sm"
                style="max-width: 115px;background: linear-gradient(-180deg, white, #adadad);"
                data-toggle="modal" data-target="#info">@lang('language.Finance Report')</button>
            <!-- Modal -->
            <div class="modal fade modal-secondary text-left" id="info"
